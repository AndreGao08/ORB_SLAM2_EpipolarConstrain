...
namespace ORB_SLAM2
{
    ...
    */
    ...
    void Tracking::CalEpipolarConstrain()
    {
        unique_lock<mutex> lock(mpMap->mMutexMapUpdate);
        if (mState == NOT_INITIALIZED || mState == NO_IMAGES_YET)
        {
            cout << "Light Trackong is not ready!" << std::endl;
        }
        // Light tracking is ready and it has velocity
        if (mState == OK && !mVelocity.empty())
        {
            // 各种阈值
            ...
            cv::Mat img_pre, img_cur;
            mCurrentFrame.mImGray.copyTo(img_cur);
            mLastFrame.mImGray.copyTo(img_pre);
            std::vector<cv::Point2f> prePoints, curPoints;
            ...
            for (int i = 0; i < mCurrentFrame.N; i++)
            {
                ...
                if (depth > 0 && depth < 7)
                {
                    curPoints.push_back(mCurrentFrame.mvKeys[i].pt);
                    validPointIndex.push_back(i);
                }
            }

            // 根据给出当前帧的特征点的坐标，计算上一帧中特征点的坐标
            cv::calcOpticalFlowPyrLK(img_cur, img_pre, curPoints, prePoints, state, err, cv::Size(22, 22), 5, cv::TermCriteria(CV_TERMCRIT_ITER | CV_TERMCRIT_EPS, 20, 0.01));
            std::vector<int> _validPointIndex;
            std::vector<cv::Point2f> _curPoints, _prePoints;
            for (int i = 0; i < state.size(); i++)
            {
                if (state[i] != 0)
                {
                    ...
                    int x1 = prePoints[i].x;
                    int y1 = prePoints[i].y;
                    int x2 = curPoints[i].x;
                    int y2 = curPoints[i].y;
                    // 要保证点在图像范围内
                    if ((x1 < limit_edge_corner || x1 >= img_cur.cols - limit_edge_corner ||
                         x2 < limit_edge_corner || x2 >= img_cur.cols - limit_edge_corner ||
                         y1 < limit_edge_corner || y1 >= img_cur.rows - limit_edge_corner ||
                         y2 < limit_edge_corner || y2 >= img_cur.rows - limit_edge_corner))
                    {
                        state[i] = 0;
                        continue;
                    }

                    // 对光流跟踪的结果进行验证，匹配中心3*3的像素差太大，就要放弃
                    double sum_check = 0;
                    for (int j = 0; j < 9; j++)
                    {
                        sum_check += abs(img_pre.at<uchar>(y1 + dy[j], x1 + dx[j]) - img_cur.at<uchar>(y2 + dy[j], x2 + dx[j]));
                    }
                    ...
                    if (state[i])
                    {
                        _validPointIndex.push_back(validPointIndex[i]);
                        _prePoints.push_back(prePoints[i]);
                        _curPoints.push_back(curPoints[i]);
                    }
                }
            }
            validPointIndex = _validPointIndex;
            prePoints = _prePoints;
            curPoints = _curPoints;

            std::vector<uchar> mask;
            std::vector<double> EpiError(mCurrentFrame.N, 0.0);

            cv::Mat F = cv::findFundamentalMat(prePoints, curPoints, mask, cv::FM_RANSAC, 0.1, 0.99);

            for (int i = 0; i < prePoints.size(); i++)
            {
                // 计算极线距离
                cv::Point2f prePoint, curPoint;
                prePoint = prePoints[i];
                curPoint = curPoints[i];

                double A = F.at<double>(0, 0) * prePoint.x + F.at<double>(0, 1) * prePoint.y + F.at<double>(0, 2);
                double B = F.at<double>(1, 0) * prePoint.x + F.at<double>(1, 1) * prePoint.y + F.at<double>(1, 2);
                double C = F.at<double>(2, 0) * prePoint.x + F.at<double>(2, 1) * prePoint.y + F.at<double>(2, 2);
                // d=|P_2*F*P_1|/sqrt(X^2+Y^2)
                double dd = fabs(A * curPoint.x + B * curPoint.y + C) / sqrt(A * A + B * B); // Epipolar constraints
                int index = validPointIndex[i];
                EpiError[index] = dd;
            }
            mCurrentFrame.mvEpiError = EpiError;
        }
    }
    /*
    ...
    */
    ...
    cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB, const cv::Mat &imD, const double &timestamp)
    {
        ...
        mCurrentFrame.mImRGB = imRGB;
        // 1. 多视角几何 判断
        LightTrack(); // 新添加的函数=======轻量级跟踪=======================
        //---new计算foundmental矩阵
        CalEpipolarConstrain();
